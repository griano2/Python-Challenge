...
let syncPairs = [];
let environments = [];  // shared reference filled by environments.js
...
function renderSyncPairsTable() {
  ...
  tbody.innerHTML = syncPairs.map((p, i) => `
    <tr data-index="${i}">
      ...
      <td class="td-mono">${p.source_environment}<br><small>${p.source_group}</small></td>
      <td>${directionLabel(p.direction)}</td>
      <td class="td-mono">${p.target_environment}<br><small>${p.target_group}</small></td>
...
function buildPairModalContent(pair = null) {
  const envOptions = getEnvNames().map(n => `<option value="${n}" ${pair?.source_environment === n ? 'selected' : ''}>${n}</option>`).join('');
  const envOptionsTarget = getEnvNames().map(n => `<option value="${n}" ${pair?.target_environment === n ? 'selected' : ''}>${n}</option>`).join('');
  ...
  return `
    <form id="pair-form" class="form-grid">
      ...
      <div class="form-group">
        <label class="form-label">Environment origen *</label>
        <select class="form-select" name="source_environment">${envOptions}</select>
      </div>
      <div class="form-group">
      ...
      <div class="form-group">
        <label class="form-label">Environment destino *</label>
        <select class="form-select" name="target_environment">${envOptionsTarget}</select>
      </div>
      <div class="form-group">
...
async function savePair() {
  ...
  const payload = {
    name: fd.get('name'),
    source_environment: fd.get('source_environment'),
    source_group: fd.get('source_group'),
    target_environment: fd.get('target_environment'),
